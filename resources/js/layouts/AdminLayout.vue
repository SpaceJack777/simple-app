...
import { MenuIcon, Bell, X } from 'lucide-vue-next'
...
const current = (current) => route().current(current)

const navigation = computed(() => [
  {
    name: 'Dashboard',
    href: route('dashboard.index'),
    current: current('dashboard.index')
  },
  {
    name: 'Simple form',
    href: route('simple-form.index'),
    current: current('simple-form.index')
  },
])
  ...
  <div class="min-h-full">
    ...
    <header class="relative bg-white shadow-sm">
      <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
        <h1 class="text-3xl font-bold tracking-tight text-gray-900">{{navigation.map(item => item.name)}}</h1>
      </div>
    </header>
    <main>
      <div class="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
